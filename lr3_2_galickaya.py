...
def md5sum(text):
    textlen = len(text)//32
    text = text + b'\x80'
    zeroPad = (448 - (textlen+8)%512)%512
    zeroPad //= 8
    text = text + b'\x00'*zeroPad + textlen.to_bytes(8,byteorder='little')
    textlen = bitlen(text)
    iterations = textlen//512
...
def hash(list):
    letter = ''.join(str(x) for x in list)
    letter = str(letter).encode("UTF-8")
    print("вх: ",letter)
    print('вых: ',md5sum(letter))

hash(['h','e','l','l','o','w','o','r','l','d'])
hash(['m','h','e','l','l','o','w','o','r','l','d'])